                ...
                if ( query.length == 1 && query[0] == '' ) {
                    ...
                    for ( var n=0; n!=query.length; ++n ) {
                        if ( query[n].search( /^(userid=[^0]|searchuser=)/ ) == 0  ) {
                            ...
                            query.splice( n--, 1 );
                        } else if ( defaults.hasOwnProperty(query[n]) ) {
                            query.splice( n--, 1 );
                        } else if ( query[n].search( /^query=/ ) == 0  ) {
                            observe_once('.tfoot', function() {
                                $(this).prepend( '<span style="float: left">Didn\'t find what you were looking for?  <a style="color: black; text-decoration: underline" href="https://www.google.com/search?q=site:forums.frontier.co.uk+' + query[n].substr(6) + '">search with Google</a></div>' );
                            });
                        }
                    }
                        ...
                        $('#posts .page').each(function() {
                            if ( $(this).has( '.smallfont a[href^="profile.php?userlist=ignore&do=removelist&u="]' ).length ) {
                                ...
                                var pm = $(this).find( '.vbmenu_popup a[href^="private.php"]'),
                                    ...
                                    quickedit = $(this).find('a[name^="vB::QuickEdit"]'),
                                ...
                                if ( quickedit ) {
                                    ...
                                    quickedit.parent().prepend( '<a style="float: left; margin-left: 1em; font-size: medium; margin-top: 4px" title="permalink to this post" href="/showthread.php?p=' + post_id + '#post' + post_id + '">[share]</a>' );
                                    if ( name ) {
                                        var parameters = '&u=' + user_id + '&username=' + encodeURIComponent(name);
                                        quickedit.parent().find('a[href^="newreply.php"]').each(function() { this.href += parameters });
                                    }
                                }
