...
document.cookie.replace( /(?:^|; *)(bb_thread_lastview=[-a-f0-9]+%7B)([^;]{4000,})%7D/, function( match, prefix, value ) {
    var times = [];
...
BabelExt.utils.dispatch(
    ...
    { // Explain different reputations and add a report button:
        match_pathname: '/usercp.php',
        match_elements: '#usercp_navpopup',
        callback: function(stash, pathname, params ) {
            $('#reputationlist li.pos').attr( 'title', 'This person approved of your post' );
            $('#reputationlist li.neg').attr( 'title', 'This person disapproved of your post' );
            $('#reputationlist li.balance').attr( 'title', "This person doesn't have enough reputation to approve or disapprove (either because they're new or they disabled their reputation)" );
            $('<a href="#report" style="float:right; margin: 0.5em" title="Report reputation abuse"><img src="images/buttons/report-40b.png"></a>')
                .insertBefore('#reputationlist li .userinfo')
                .each(function() {
                    var $li      = $(this).closest('li');
                    var rep_type = $li.hasClass('pos') ? 'positive (green)' : $li.hasClass('neg') ? 'negative (red)' : 'balanced (grey)';
                    var post     = $li.find( '.title a' );
                    var message  = $li.find( '.comment' );
                    var giver    = $li.find('.user');
                    var time     = $li.find('.datetime');
                    this.href = '/report.php?p=' + post.attr('href').split('#post')[1] + '&message=' + encodeURIComponent(
                        "My post was given an abusive piece of " + rep_type + " reputation at " + time.text() + ":\n" +
                        '\n' +
                        '[quote=' + $.trim(giver.text()) + ']' + message.text() + '[/quote]\n' +
                        '\n' +
                        '(please explain why this message crosses the line from abrasive feedback to personal abuse)'
                    );
                });
        }
    },

    {
        ...
        match_elements: '#message',
        callback: function(stash, pathname, params, message) {
    ...
    { // "Caution" box
        ...
        callback: function(stash, pathname, params, subject, editor, save) {
            ...
            $('#subject,#title,#cke_contents_vB_Editor_001_editor').on( 'input', function() {
                ...
                if ( !reason && (params['do']||'').search(/new/) == 0 ) // new thread, new reply
...
BabelExt.utils.dispatch(
    { // GMT/BST to local time
        match_pathname: '/showthread.php',
        match_elements: ['#below_postlist'],
        callback: function(stash, pathname, params) {
            $('div.postrow').each(function() {
                var dates = {}, has_dates = false;
                $(this).text().replace( /\b([012]?[0-9])(?::([0-6][0-9]))?\s*(am|pm|)\s*(gmt|bst)\b/gi, function( match, hour, minute, ap, zone ) {
                    var minutes_past_midnight_local_time = (
                        0
                            + parseInt(hour       , 10) * 60
                            + parseInt(minute||'0', 10)
                            + ( ( ap  .toLowerCase() == 'pm'  ) ? 60*12 : 0 )
                            - ( ( zone.toLowerCase() == 'bst' ) ? 60    : 0 )
                            - new Date().getTimezoneOffset()
                    ) % (60*60*24);
                    var local_hour = Math.floor(minutes_past_midnight_local_time/60);
                    var local_minute = minutes_past_midnight_local_time % 60;
                    if ( local_hour   < 10 ) local_hour   = '0' + local_hour  ;
                    if ( local_minute < 10 ) local_minute = '0' + local_minute;
                    has_dates = true;
                    dates[ '<li>' + match + ' is ' + local_hour + ':' + local_minute + ' your time' ] = 1;
                });
                if ( has_dates )
                    $(this).after('<ol class="notices">' + Object.keys(dates).sort().join("") + '</ol>');
            });
        }
    },

    { // Show relative dates and link to change time zone
        match_elements: [ '#footer_time' ],
        callback: function(stash, pathname, params, footer_time) {
            function parse_time(time) { // convert a time to a number of minutes past midnight
                time = time.match(/^([0-9]{1,2}):([0-9]{1,2}) ([AP])M$/);
                return parseInt(time[1],10)*60 + parseInt(time[2],10) + ( time[3]=='A' ? 0 : 12*60 );
            }

            var current_time = parse_time( $(footer_time).find('.time').text() );
            $('.time').not().each(function() {
                if ( this.parentNode.id == 'footer_time' ) return;
                var time_offset = current_time - parse_time( $(this).text() );
                if ( $(this).parent().text().search( /Yesterday/ ) > -1 )
                    time_offset += 60*24;
                else if ( $(this).parent().text().search( /Today/ ) == -1 )
                    return; // neither today nor yesterday - too long ago
                if      ( time_offset == 0 ) $(this).after( ' (less than a minute ago)' );
                else if ( time_offset == 1 ) $(this).after( ' (about a minute ago)' );
                else if ( time_offset < 60 ) $(this).after( ' (' + time_offset + ' minutes ago)' );
            });

            $(footer_time).append( ' <a href="/profile.php?do=editoptions#sel_timezoneoffset">[edit time zone]</a>' );
        }
    }
